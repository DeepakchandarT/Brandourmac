...
export default function AdminDashboard({initialAnalytics,initialDraft,initialPublished,initialCampaignOpen}:{initialAnalytics:Analytics;initialDraft:SponsorConfig;initialPublished:SponsorConfig;initialCampaignOpen:boolean}){
  ...
  const [name,setName]=useState(draft.name),[website,setWebsite]=useState(draft.website),[fundsRaised,setFundsRaised]=useState(String(draft.fundsRaised)),[fundingCurrency,setFundingCurrency]=useState(draft.fundingCurrency),[logoPreview,setLogoPreview]=useState(draft.logoDataUrl),[busy,setBusy]=useState(""),[message,setMessage]=useState("");
  ...
  async function save(event:FormEvent){event.preventDefault();setBusy("save");setMessage("");const response=await fetch("/api/admin/sponsor",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,website,fundsRaised,fundingCurrency})});const result=await response.json();if(response.ok){setDraft(result);setMessage("Draft saved. Review it, then publish.");}else setMessage(result.error);setBusy("");}
  ...
  return <main className="admin-shell">
    <aside className="admin-sidebar"><a href="/" className="admin-wordmark" aria-label="BrandMyReach home"><Image src="/brand/brandmyreach-wordmark.png" alt="BrandMyReach" width={190} height={42}/></a><nav><a href="#overview">Overview</a><a href="#traffic">Traffic</a><a href="#sponsor">Sponsor manager</a></nav><button onClick={logout}><LogOut size={15}/> Sign out</button></aside>
    <div className="admin-content">
      ...
      <section id="sponsor" className="admin-panel sponsor-manager"><div className="admin-section-heading"><div><h2>Sponsor manager</h2><p>Save a private draft, preview it, then publish.</p></div><span className="published-state">Live: {published.name}</span></div>
        <div className="sponsor-grid"><form onSubmit={save} className="sponsor-form">
          ...
          <div className="sponsor-funding-fields"><label>Confirmed sponsorship amount<input value={fundsRaised} onChange={e=>setFundsRaised(e.target.value)} required type="number" min="0" max="999999999999" step="0.01" inputMode="decimal"/></label><label>Currency<select value={fundingCurrency} onChange={e=>setFundingCurrency(e.target.value as "USD"|"EUR"|"INR")}><option value="USD">USD</option><option value="EUR">EUR</option><option value="INR">INR</option></select></label></div>
